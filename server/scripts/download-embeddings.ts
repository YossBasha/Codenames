...
const EMBEDDINGS_URL =
  "https://raw.githubusercontent.com/devmount/glove-word-vectors/master/glove.6B.50d.json";

function stableVector(word: string): number[] {
  let hash = 2166136261;
...
function generateFallbackEmbeddings() {
  console.log(
    "⚠️ Failed to download real embeddings. Generating fallback dictionary...",
  );
  ...
  for (const pack in wordPackRegistry.en) {
    ...
    words.push(
      ...wordPackRegistry.en[pack].map((w: string) => w.toLowerCase()),
    );
  }
  ...
  console.log(
    "❌ WARNING: AI bots are using a deterministic fallback embedding set. Provide a real GloVe JSON file for higher-quality clues.",
  );
}
    ...
    .get(EMBEDDINGS_URL, (res: any) => {
      ...
      res.on("end", () => {
        try {
          ...
          console.log(
            "✅ Successfully downloaded and saved real word embeddings!",
          );
        } catch (err: any) {
