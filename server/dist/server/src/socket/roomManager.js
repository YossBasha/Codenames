...
function applyRandomModifier(room) {
    ...
    const availableModifiers = modifiers_1.MODIFIERS.filter((m) => enabled.includes(m.id));
...
function setupRoomManager(io) {
    io.on("connection", (socket) => {
        ...
        socket.on("join_room", ({ roomId, player, explicitChange, isPublic, }) => {
            socket.join(roomId);
            if (!rooms[roomId]) {
                rooms[roomId] = {
                    id: roomId,
