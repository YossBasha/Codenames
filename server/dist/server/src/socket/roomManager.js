...
exports.getLocalRoomsForRegistration = getLocalRoomsForRegistration;
exports.registerRooms = registerRooms;
exports.getPublicRooms = getPublicRooms;
exports.setupRoomManager = setupRoomManager;
const gameLogic_1 = require("../../../shared/gameLogic");
const modifiers_1 = require("../../../shared/modifiers");
const wordPacks_1 = require("../../../shared/wordPacks");
const aiLogic_1 = require("../utils/aiLogic");
const rooms = {};
function getPlayerAvatarUrl(player, defaultBg) {
    // Never embed huge base64 images in logs, it crashes bandwidth over websockets
    if (player.avatarBase64 && !player.avatarBase64.startsWith('data:image'))
        return player.avatarBase64;
...
function getLoggedWord(room, card) {
    if (room.gameState &&
...
const registeredRooms = new Map();
function getLocalRoomsForRegistration() {
...
function registerRooms(serverUrl, roomsList) {
...
function stopRogueAssassinInterval(room) {
...
function startRogueAssassinInterval(io, room) {
    stopRogueAssassinInterval(room);
    if (!room.gameState || room.gameState.winner)
        return;
    if (room.gameState.activeModifier !== "rogue-assassin")
        return;
    room.rogueAssassinInterval = setInterval(() => {
        ...
        if (isDuet) {
            ...
            const guesserKey = expectedGuessTeam === "blue" ? "duetTypeA" : "duetTypeB";
            const revealedKey = expectedGuessTeam === "blue" ? "revealedByB" : "revealedByA";
            // Swap cards that are unrevealed for the guesser team
            const unrevealedAssassins = state.cards.filter((c) => !c[revealedKey] && c[guesserKey] === "assassin");
            const unrevealedNeutrals = state.cards.filter((c) => !c[revealedKey] && c[guesserKey] === "neutral");
...
function getBotDelayMs(room) {
    if (room.gameState?.chaosMode) {
        return 5000;
    }
    return 2000;
}
function revertActiveModifier(room) {
    const gameState = room.gameState;
    if (!gameState || !gameState.activeModifier)
        return;
    ...
    else if (modifier === "lost-in-translation" ||
...
function applyRandomModifier(room) {
    const gameState = room.gameState;
    if (!gameState)
        return;
    if (gameState.activeModifier)
        return;
    const enabled = gameState.enabledModifiers || modifiers_1.MODIFIERS.map((m) => m.id);
    const availableModifiers = modifiers_1.MODIFIERS.filter((m) => {
        if (!enabled.includes(m.id))
            return false;
        if (m.id === "nimnims-bite" && gameState.nimnimsCooldown && gameState.nimnimsCooldown > 0)
            return false;
        return true;
    });
    ...
    if (randomModifier.id === "dimensional-scramble") {
        ...
        if (unrevealedIndices.length >= 3) {
            const shuffledIndices = (0, gameLogic_1.shuffleArray)([...unrevealedIndices]);
        ...
        gameState.cards.forEach((card) => {
            if (card.revealed)
                return;
            const sourcePacks = wordPacks_1.wordPackRegistry[gameState.language];
            const targetPacks = wordPacks_1.wordPackRegistry[targetLang];
            let found = false;
        ...
        if (unrevealedCards.length >= 2) {
            const shuffled = (0, gameLogic_1.shuffleArray)([...unrevealedCards]);
            gameState.modifierState = {
        ...
        const shuffledWords = (0, gameLogic_1.shuffleArray)([...unrevealedWords]);
        const unrevealedData = unrevealedCards.map((c) => ({
            word: c.word,
            type: c.type,
        ...
        const shuffledData = (0, gameLogic_1.shuffleArray)([...unrevealedData]);
        let shuffleIdx = 0;
        gameState.cards.forEach((c) => {
            if (!c.revealed) {
                ...
                c.word = data.word;
                c.type = data.type;
                c.duetTypeA = data.duetTypeA;
                c.duetTypeB = data.duetTypeB;
        ...
        const shuffled = (0, gameLogic_1.shuffleArray)([...unrevealedIndices]);
...
function generateGachaChances(gameState) {
    const isDuet = gameState.gameMode === "duet";
    const expectedGuessTeam = isDuet
        ? gameState.currentTurn === "red"
            ? "blue"
    ...
    gameState.cards.forEach((c) => {
        if (isDuet) {
            ...
            if (!c.revealed) {
                if (c.type === expectedGuessTeam)
                    hasCorrect = true;
                else if (c.type === "assassin")
                    hasAssassin = true;
    ...
    if (total === 0)
        return null;
    let sum = 0;
    for (const k of ["correct", "assassin", "enemy", "neutral"]) {
...
function transitionToNewTurn(io, room) {
    const gameState = room.gameState;
    if (!gameState)
        return;
    revertActiveModifier(room);
    let nextTurn = gameState.gameMode === "duet"
        ? getNextTurnDuet(gameState)
        : gameState.currentTurn === "red"
            ? "blue"
            : "red";
    // --- Intercept Penalty: skip this team's turn ---
    if (gameState.modifierState?.interceptPenalty === nextTurn &&
    ...
    if (gameState.currentPhase === "spymaster") {
        const spymasterBots = room.players.filter((p) => p.team === gameState.currentTurn && p.role === "spymaster" && p.isBot);
        if (spymasterBots.length > 0) {
            setTimeout(() => triggerBotSpymaster(io, room, spymasterBots[0]), getBotDelayMs(room));
        }
    }
}
async function triggerBotSpymaster(io, room, bot) {
    const gameState = room.gameState;
    ...
    try {
        ...
        const clue = await (0, aiLogic_1.getLLMSpymasterClue)(gameState.cards, bot.team, gameState.language, gameState.gameMode === "duet", gameState.activeModifier, gameState.modifierState);
        ...
        if (clue) {
            ...
            if (room.gameState.activeModifier === "the-intercept" &&
                ...
                const interceptInterval = setInterval(() => {
                    ...
                    if (room.gameState.modifierState.interceptTimeLeft <= 0) {
                        ...
                        clearInterval(interceptInterval);
                        startTimer(io, room);
                        const operativeBots = room.players.filter((p) => p.team === room.gameState.currentTurn &&
                            p.role === "operative" &&
                            p.isBot);
                        if (operativeBots.length > 0) {
                            setTimeout(() => triggerBotOperatives(io, room, room.gameState.activeCue || "", room.gameState.activeCueNumber || 1, operativeBots[0]), getBotDelayMs(room));
                        }
                    }
            ...
            const operativeBots = room.players.filter((p) => p.team ===
            ...
            if (operativeBots.length > 0) {
                setTimeout(() => triggerBotOperatives(io, room, clue.word, clue.count, operativeBots[0]), getBotDelayMs(room));
            }
        }
        else {
...
async function triggerBotOperatives(io, room, clue, count, bot) {
    const gameState = room.gameState;
    ...
    try {
        ...
        const { cards: rankedCards, reasoning } = await (0, aiLogic_1.getLLMOperativeRankings)(clue, count, gameState.cards, bot.team, gameState.language, gameState.gameMode === "duet", gameState.activeModifier, gameState.modifierState);
        ...
        function nextGuess() {
            ...
            const stillUnrevealed = currentGameState.gameMode === "duet"
                ? bot.team === "blue"
...
function startTimer(io, room) {
    ...
    const isHaste = room.gameState.activeModifier === "haste" &&
        room.gameState.currentPhase === "operative";
    if (room.gameState.timerSettings.preset === "off" && !isHaste)
        return;
    let duration = 0;
    if (isHaste) {
        duration = 15;
    ...
    room.timerInterval = setInterval(() => {
        ...
        const isTimerFrozen = (room.gameState.activeModifier === "critical-hit" &&
        ...
        if (room.gameState.timeRemaining <= 0) {
            ...
            if (room.gameState.currentPhase === "spymaster") {
                ...
                room.gameState.gameLog.push({
                    ...
                    guessingTeam: room.gameState.currentTurn,
...
function getNextTurnDuet(gameState) {
    let nextTurn = gameState.currentTurn === "red" ? "blue" : "red";
    if (nextTurn === "red") {
        const aRemaining = gameState.cards.filter((c) => c.duetTypeA === "green" && !c.revealed).length;
        if (aRemaining === 0)
            return "blue";
    }
    else {
        const bRemaining = gameState.cards.filter((c) => c.duetTypeB === "green" && !c.revealed).length;
        if (bRemaining === 0)
            return "red";
    }
    return nextTurn;
}
function processGuess(io, room, player, cardId) {
    if (!room.gameState || room.gameState.winner)
        return;
    ...
    if (room.gameState.activeModifier === "slippery-fingers" &&
        Math.random() < 0.25) {
    ...
    if (isDuet) {
        ...
        if (expectedGuessTeam === "blue")
            card.revealedByB = true;
        else
            card.revealedByA = true;
        if (room.gameState.highlightedCards) {
            for (const playerId of Object.keys(room.gameState.highlightedCards)) {
                room.gameState.highlightedCards[playerId] =
                    room.gameState.highlightedCards[playerId].filter((cId) => cId !== cardId);
            }
        }
        if (keyType === "green" ||
            keyType === "assassin" ||
        ...
        room.gameState.gameLog.push({
            ...
            guessingTeam: expectedGuessTeam,
        ...
        if (keyType === "assassin") {
            ...
            if (room.gameState.activeModifier === "critical-hit" &&
                room.gameState.successfulGuessesThisTurn === 0 &&
            ...
            if (foundGreens >= 15) {
                ...
                if (expectedGuessTeam === "blue") {
                    remainingTargetsForCurrentTeam = room.gameState.cards.filter((c) => c.duetTypeA === "green" && !c.revealed).length;
                }
                else {
                    remainingTargetsForCurrentTeam = room.gameState.cards.filter((c) => c.duetTypeB === "green" && !c.revealed).length;
                }
                if (remainingTargetsForCurrentTeam === 0 ||
...
function setupRoomManager(io) {
    io.on("connection", (socket) => {
        console.log(`Client connected: ${socket.id}`);
        socket.on("join_room_as_observer", (roomId) => {
            socket.join(roomId);
            if (!rooms[roomId]) {
                rooms[roomId] = {
                    id: roomId,
        ...
        socket.on("join_room", ({ roomId, player, explicitChange, isPublic, appVersion, }) => {
            socket.join(roomId);
            if (!rooms[roomId]) {
                rooms[roomId] = {
                    id: roomId,
                ...
                if (appVersion &&
                    ...
                    const isNewPlayer = !rooms[roomId].players.some((p) => (player.sessionId && p.sessionId === player.sessionId) ||
                        p.id === player.id);
                    if (isNewPlayer) {
            ...
            const existingPlayerIndex = rooms[roomId].players.findIndex((p) => (player.sessionId && p.sessionId === player.sessionId) ||
                p.id === player.id);
            if (existingPlayerIndex !== -1) {
        ...
        socket.on("add_bot", ({ roomId, team, role, }) => {
            if (!rooms[roomId])
                return;
        ...
        socket.on("remove_bot", ({ roomId, botId }) => {
            if (!rooms[roomId])
                return;
            rooms[roomId].players = rooms[roomId].players.filter((p) => p.id !== botId || !p.isBot);
            io.to(roomId).emit("room_update", getSafeRoom(rooms[roomId]));
        });
        ...
        socket.on("update_custom_words_text", ({ roomId, text }) => {
            const room = rooms[roomId];
        ...
        socket.on("randomize_teams", ({ roomId }) => {
            const room = rooms[roomId];
            if (room) {
                const shuffled = (0, gameLogic_1.shuffleArray)([...room.players]);
        ...
        socket.on("start_game", ({ roomId, language, gameMode, timerSettings, selectedPacks, customWords, customWordWeight, clueType, chaosMode, enabledModifiers, }) => {
            const room = rooms[roomId];
            if (room) {
                const { cards, startingTeam } = gameMode === "duet"
                    ? (0, gameLogic_1.generateDuetGrid)(language, selectedPacks, customWords, customWordWeight)
                    : (0, gameLogic_1.generateGrid)(language, selectedPacks, customWords, customWordWeight);
                ...
                room.gameState = {
                    ...
                    clueType,
                    chaosMode: !!chaosMode,
                    enabledModifiers: enabledModifiers || modifiers_1.MODIFIERS.map((m) => m.id),
                };
                if (chaosMode) {
                    applyRandomModifier(room);
                }
                ...
                if (room.gameState.currentPhase === "spymaster") {
                    const spymasterBots = room.players.filter((p) => p.team === room.gameState.currentTurn &&
                        p.role === "spymaster" &&
                        p.isBot);
                    if (spymasterBots.length > 0) {
                        setTimeout(() => triggerBotSpymaster(io, room, spymasterBots[0]), getBotDelayMs(room) + 1000);
                    }
                }
            }
        });
        socket.on("submit_cue", ({ roomId, cue, number, targets, }) => {
            const room = rooms[roomId];
            ...
            if (room && room.gameState && !room.gameState.winner && player) {
                ...
                if (room.gameState.currentPhase === "spymaster" &&
                    ...
                    if (room.gameState.activeModifier === "vowel-void") {
                        ...
                        if (words.length !== 2 ||
                            !(0, modifiers_1.checkRhyme)(words[0], words[1], !!room.gameState.isRTL)) {
                            return;
                        }
                        finalCue = words.join(" ");
                    }
                    ...
                    room.gameState.gameLog.push({
                        ...
                        targets: targets,
                        targetWords: targets?.map(id => room.gameState.cards.find(c => c.id === id)?.word).filter(Boolean),
                        modifier: room.gameState.activeModifier || null,
                        timestamp: Date.now(),
                    });
                    if (room.gameState.activeModifier === "the-intercept" &&
                    ...
                    if (room.gameState.activeModifier !== "the-intercept" ||
                        ...
                        const operativeBots = room.players.filter((p) => p.team === room.gameState.currentTurn &&
                            ...
                            p.isBot);
                        if (operativeBots.length > 0) {
                            setTimeout(() => triggerBotOperatives(io, room, finalCue, finalNumber, operativeBots[0]), getBotDelayMs(room));
                        }
                    }
                }
            }
        });
        socket.on("highlight_card", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
        ...
        socket.on("guess_card", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && player) {
        ...
        socket.on("reject_clue", ({ roomId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
                    return;
                if (room.gameState.activeModifier !== "mutiny")
                    return;
                if (room.gameState.modifierState?.mutinyUsed)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedGuessTeam = isDuet
                    ? room.gameState.currentTurn === "red"
        ...
        socket.on("report_cheat", ({ roomId, clueId }) => {
            console.log(`[report_cheat] Received for roomId: ${roomId}, clueId: ${clueId}`);
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (!room)
                console.log(`[report_cheat] Room not found.`);
            if (!room?.gameState)
                console.log(`[report_cheat] GameState not found.`);
            if (!player)
                console.log(`[report_cheat] Player not found for socket id: ${socket.id}`);
            if (room && room.gameState && player) {
                const clueEntry = room.gameState.gameLog.find(l => l.type === 'cue' && l.id === clueId);
                if (!clueEntry) {
        ...
        socket.on("vote_cheat", ({ roomId, vote }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
        ...
        socket.on("resolve_cheat", ({ roomId, isCheat }) => {
            const room = rooms[roomId];
            if (room && room.gameState && room.gameState.cheatVoteState) {
                const clueEntry = room.gameState.gameLog.find(l => l.id === room.gameState.cheatVoteState.clueId);
                if (isCheat) {
                    if (clueEntry) {
        ...
        socket.on("intercept_guess", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
                ...
                if (room.gameState.activeModifier !== "the-intercept")
                    return;
                if (!room.gameState.modifierState?.interceptPhase)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                if (isDuet)
                    return;
                ...
                if (player.team !== enemyTeam || player.role !== "operative")
                    return;
        ...
        socket.on("use_blood_pact", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
                    return;
                if (room.gameState.activeModifier !== "blood-pact")
                    return;
                if (room.gameState.modifierState?.bloodPactStatus !== "available")
                    return;
                if (room.gameState.successfulGuessesThisTurn > 0)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedGuessTeam = isDuet
                    ? room.gameState.currentTurn === "red"
                ...
                if (!card)
                    return;
                if (isDuet) {
                    const alreadyRevealed = expectedGuessTeam === "blue"
                        ? card.revealedByB
                        : card.revealedByA;
                    if (alreadyRevealed)
                        return;
                ...
                if (!isDuet) {
                    ...
                    let foundGreens = 0;
                    room.gameState.cards.forEach((c) => {
                        if (c.duetTypeA === "green" && c.revealedByB)
                            foundGreens++;
                        if (c.duetTypeB === "green" && c.revealedByA)
                            foundGreens++;
                    });
                    if (foundGreens >= 15) {
                        room.gameState.winner = "red";
        ...
        socket.on("d20_roll", ({ roomId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "spymaster")
                    return;
                if (room.gameState.activeModifier !== "d20-roll")
                    return;
                if (room.gameState.modifierState?.rolled)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedTurn = room.gameState.currentTurn;
                if (player.team !== expectedTurn ||
        ...
        socket.on("d20_free_reveal", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "spymaster")
                ...
                if (room.gameState.activeModifier !== "d20-roll")
                    return;
                if (!room.gameState.modifierState?.canRevealForFree)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedTurn = room.gameState.currentTurn;
                if (player.team !== expectedTurn ||
                ...
                if (isDuet) {
                    ...
                    const teamCards = room.gameState.cards.filter((c) => c.type === teamColor);
        ...
        socket.on("gacha_pull", ({ roomId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
                    return;
                if (room.gameState.activeModifier !== "gacha-pull")
                    return;
                if (room.gameState.modifierState?.gachaPulling)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedGuessTeam = isDuet
                    ? room.gameState.currentTurn === "red"
                ...
                if (unrevealedCards.length > 0) {
                    ...
                    const validCardsForType = unrevealedCards.filter((c) => {
                        ...
                        if (isDuet) {
                            ...
                            if (selectedType === "assassin")
                                return c.type === "assassin";
                            if (selectedType === "neutral")
                                return c.type === "neutral";
                    ...
                    if (validCardsForType.length > 0) {
                        randomCard =
                            validCardsForType[Math.floor(Math.random() * validCardsForType.length)];
                    }
                    else {
                        randomCard =
                            unrevealedCards[Math.floor(Math.random() * unrevealedCards.length)];
                    }
        ...
        socket.on("lock_card", ({ roomId, cardId }) => {
            const room = rooms[roomId];
            const player = room?.players.find((p) => p.id === socket.id);
            if (room && room.gameState && !room.gameState.winner && player) {
                if (room.gameState.currentPhase !== "operative")
                    return;
                if (room.gameState.activeModifier !== "shield-wall")
                    return;
                if (!room.gameState.modifierState?.shieldActive)
                    return;
                const isDuet = room.gameState.gameMode === "duet";
                const expectedGuessTeam = isDuet
                    ? room.gameState.currentTurn === "red"
        ...
        socket.on("prank_vibrate", ({ roomId, targetPlayerId, }) => {
            const room = rooms[roomId];
            if (room &&
            ...
            for (const roomId in rooms) {
                ...
                if (playerIndex !== -1) {
                    ...
                    if (isHost && !room.returningToLobby) {
                        console.log(`Host ${player.name} disconnected from room ${roomId}. Scheduling room deletion with 5s grace period.`);
                        ...
                        console.log(`Host ${player.name} disconnected during lobby transition for room ${roomId}. Skipping deletion – expecting reconnect.`);
                    }
                }
            }
        });
