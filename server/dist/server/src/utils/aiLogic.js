...
exports.getEnglishEquivalent = getEnglishEquivalent;
exports.getWordVector = getWordVector;
exports.getAllDictionaryWords = getAllDictionaryWords;
exports.getBotSpymasterClue = getBotSpymasterClue;
exports.rankCardsForOperative = rankCardsForOperative;
exports.getLLMSpymasterClue = getLLMSpymasterClue;
exports.getLLMOperativeRankings = getLLMOperativeRankings;
...
const groq = process.env.GROQ_API_KEY
    ...
    : null;
console.log(`[AI-INIT] GROQ_API_KEY present: ${!!process.env.GROQ_API_KEY}, Groq client ready: ${!!groq}`);
let embeddings = {};
...
function getEnglishEquivalent(word, language) {
    if (language === "en")
        return word.toLowerCase();
    for (const packName in wordPacks_1.wordPackRegistry[language]) {
        const idx = wordPacks_1.wordPackRegistry[language][packName].indexOf(word);
        if (idx !== -1 &&
            wordPacks_1.wordPackRegistry["en"] &&
            wordPacks_1.wordPackRegistry["en"][packName] &&
            wordPacks_1.wordPackRegistry["en"][packName][idx]) {
            return wordPacks_1.wordPackRegistry["en"][packName][idx].toLowerCase();
        }
    }
    // Fallback if it's a custom word or not found
    return word.toLowerCase();
}
function getWordVector(word, language) {
...
function applyModifierRules(clueWord, clueCount, activeModifier, modifierState, candidateWords) {
    ...
    if (activeModifier === "five-letter-curse") {
        const exactLengthWord = candidateWords.find((word) => word.length === 5 && !nextWord.toLowerCase().includes(word));
        if (exactLengthWord) {
    ...
    if (activeModifier === "boolean-search") {
        ...
        const fallbackWord = candidateWords.find((word) => word !== nextWord.toLowerCase()) || "thing";
    ...
    if (activeModifier === "forced-acronym") {
        ...
        if (letters.length > 0) {
            const phraseWords = letters
                .map((letter) => candidateWords.find((word) => word.startsWith(letter) &&
 ...
 */
function getBotSpymasterClue(cards, team, language, isDuet, activeModifier = null, modifierState = null) {
    ...
    unrevealedCards.forEach((c) => {
        ...
        if (isDuet) {
            if (type === "green")
                friendlyCards.push(c);
            else if (type === "assassin")
                assassinCards.push(c);
    ...
    const boardWords = new Set(cards.map((c) => getEnglishEquivalent(c.word, language).toLowerCase()));
    const allDictWords = getAllDictionaryWords().filter((w) => !boardWords.has(w.toLowerCase()) && w.length > 1 && !w.includes(" "));
    ...
    if (allDictWords.length === 0 && HARDCODED_FALLBACK_WORDS.length > 0) {
        const word = HARDCODED_FALLBACK_WORDS[Math.floor(Math.random() * HARDCODED_FALLBACK_WORDS.length)];
        return { word, count: 1 };
    }
    ...
    for (let targetCount = maxTargets; targetCount >= 1; targetCount--) {
        ...
        const bestFriendlySubset = selectBestCardSubset(friendlyCards, language, targetCount);
        // === PHASE 5: GENERATE CANDIDATE CLUES ===
        const candidates = generateRefinedCandidates(bestFriendlySubset, language, allDictWords);
        // === PHASE 6: SCORE AND FIND BEST CLUE ===
        let bestScore = -Infinity;
        let bestCandidate = null;
        for (const candidateWord of candidates) {
            const score = scoreClueCandidate(candidateWord, bestFriendlySubset, neutralCards, enemyCards, assassinCards, language);
        ...
        if (bestCandidate && bestScore > -10) {
            const finalClue = applyModifierRules(bestCandidate, targetCount, activeModifier, modifierState, allDictWords);
            return finalClue;
        }
    }
    ...
    if (allDictWords.length > 0) {
        const randomWord = allDictWords[Math.floor(Math.random() * allDictWords.length)];
        const fallbackClue = applyModifierRules(randomWord, 1, activeModifier, modifierState, allDictWords);
        return fallbackClue
            ? {
                ...fallbackClue,
 ...
 */
function selectBestCardSubset(friendlyCards, language, targetCount) {
 ...
 */
function generateRefinedCandidates(targetCards, language, allDictWords) {
 ...
 */
function scoreClueCandidate(candidateWord, targetCards, neutralCards, enemyCards, assassinCards, language) {
...
function rankCardsForOperative(clue, cards, team, language, isDuet, activeModifier = null, modifierState = null) {
...
function findBestCluster(friendlyCards, language) {
...
async function getLLMSpymasterClue(cards, team, language, isDuet, activeModifier = null, modifierState = null) {
    console.log(`[AI-SPY] getLLMSpymasterClue called. Team: ${team}, groqReady: ${!!groq}, modifier: ${activeModifier}`);
    if (!groq) {
        console.warn(`[AI-SPY] No Groq client — falling back to local AI.`);
        return getBotSpymasterClue(cards, team, language, isDuet, activeModifier, modifierState);
    }
    const unrevealedCards = cards.filter((c) => {
        if (activeModifier === "nimnims-bite" && modifierState?.eatenCardIds?.includes(c.id)) {
            return false;
    ...
    unrevealedCards.forEach((c) => {
        ...
        if (isDuet) {
            if (type === "green")
                ...
                assassinWords.push(c.word);
            else if (type === "neutral")
                neutralWords.push(c.word);
        }
        else {
            if (type === team)
                friendlyCards.push(c);
        ...
        if (content) {
            ...
            if (parsed.clue && parsed.count !== undefined) {
                ...
                if (finalClue.includes(" ")) {
                    console.warn(`[AI-SPY] LLM returned a phrase "${finalClue}" — taking first word.`);
                    finalClue = finalClue.split(" ")[0];
                }
                if (!isNaN(finalCount)) {
                    console.log(`[AI-SPY] Valid clue parsed: "${finalClue}" x${finalCount}`);
                    return {
                        word: finalClue,
                        count: finalCount,
                    ...
                    console.warn(`[AI-SPY] Count could not be parsed as a number:`, parsed.count);
                }
            }
            else {
                console.warn(`[AI-SPY] Parsed JSON but missing clue/count fields:`, parsed);
            }
        }
    }
    catch (err) {
...
async function getLLMOperativeRankings(clue, count, cards, team, language, isDuet, activeModifier = null, modifierState = null) {
    console.log(`[AI-OP] getLLMOperativeRankings called. Team: ${team}, clue: "${clue}" x${count}, groqReady: ${!!groq}`);
    if (!groq) {
...
Rank the available words by how well they match the clue's semantic meaning. Include ALL available words in your ranking. Output ONLY JSON.`;
        ...
        if (content) {
            ...
            if (Array.isArray(parsed.rankedWords)) {
                ...
                for (const w of parsed.rankedWords) {
                    const card = unrevealed.find((c) => c.word.toLowerCase() === w.toLowerCase());
                    if (card)
                ...
                console.log(`[AI-OP] Final ranked cards (${rankedCards.length}):`, rankedCards.slice(0, 5).map((c) => c.word));
                return {
                    cards: rankedCards,
                    reasoning: parsed.reasoning || "No reasoning provided.",
                };
                ...
                console.warn(`[AI-OP] Parsed JSON but missing rankedWords array:`, parsed);
            }
        }
    }
    catch (err) {
    ...
    // Fallback
    console.warn(`[AI-OP] Falling back to local operative ranking after Groq failure.`);
    return {
        cards: rankCardsForOperative(clue, cards, team, language, isDuet, activeModifier, modifierState),
